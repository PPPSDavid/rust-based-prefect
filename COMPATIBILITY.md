...
- Not yet supported (open gaps — see **`docs/plans/prefect-gap-canvas.md`** for prioritized backlog; not parity claims):
  ...
  - **Force cancel / operator pause-resume** (planned — `docs/plans/flow-run-lifecycle-control.md`): cancel should **terminate** running task workers; pause supports explicit **`drain`** vs **`terminate`**. Today: control-plane cancel is recorded; bodies must poll (`sleep_cancelable`) unless process-kill lands; deployment `paused` ≠ flow-run pause; gate-driven `PAUSED` is calendar wait only.
