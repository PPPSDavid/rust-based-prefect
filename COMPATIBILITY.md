...
- Supported:
  ...
  - **Docker Compose + services (Tier B3/B5 subset):** `deploy/docker/compose.yml` — Postgres + API (`ENABLE_SCHEDULER/LOCAL_WORKER=0`) + `ironflow server services start` + HTTP worker; GHA `docker-compose-smoke.yml`. HA multi-services leader election and Redis deferred. See **`docs/how-to/docker-compose.md`**.
