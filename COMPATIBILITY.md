...
- Supported:
  ...
  - `task.submit()` dependency chains; with **`ThreadPoolTaskRunner`** (default) or **`ProcessPoolTaskRunner`**, independent submits return a future immediately (PENDING task run + planned node). `wait_for`, tag-slot acquire, PENDING→RUNNING, and the body run on a worker (thread orchestrator; process bodies for the process runner). If `wait_for` / start fails before RUNNING, the PENDING run is closed as **CANCELLED** (body never ran). **`SequentialTaskRunner`** keeps submit end-to-end synchronous on the coordinating thread.
  ...
  - **Tag-based concurrency limits (subset):** `@task(tags=...)`; limits named `tag:{tag}` (via `create_tag_concurrency_limit` or GCL CRUD); AND across tags on enter `Running`; limit `0` aborts (`CANCELLED` + error). Tag wait poll: `IRONFLOW_TASK_TAG_SLOT_WAIT_SECONDS`. Thread/process `submit` and `map` acquire per worker so fan-out respects the cap without blocking the coordinating-thread submit return.
  - **State transition hooks** (IronFlow extension, not Prefect API names): pass `transition_hooks=` to `@flow` / `@task` as a sequence of `TransitionHookSpec` from `on_transition(fn, from_state=..., to_state=...)`. `None` for `from_state` or `to_state` is a wildcard. Hooks run **synchronously in-process** after each successful control-plane transition (including PENDING and RUNNING edges on sequential batched start and on deferred concurrent promote), **without** holding the control-plane lock. User hook bodies may block arbitrarily; IronFlow only guarantees low overhead when **no** hooks are registered. Hook exceptions are logged and do not fail the run. Prefect’s separate `on_running` / `on_failure` / … style maps to explicit edges (e.g. `PENDING→RUNNING`, any `→FAILED`).
