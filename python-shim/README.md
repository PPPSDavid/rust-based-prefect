...
This PyPI distribution bundles **prebuilt native wheels** per platform where CI publishes them (**CPython 3.11–3.14**). You **do not** need Rust installed to **use** those wheels; Rust is used **when building** the package from source (for example **sdist** installs when no wheel matches).
