"""Pluggable task runners for FlowOxide (Prefect-shaped API)."""
...
class ThreadPoolTaskRunner:
    ...
    def resolve_max_workers(self) -> int:
        ...
        env = os.environ.get("FLOWOXIDE_TASK_RUNNER_THREAD_POOL_MAX_WORKERS")
...
class ProcessPoolTaskRunner:
    ...
    def resolve_max_workers(self) -> int:
        if self.max_workers is not None:
            return max(1, self.max_workers)
        env = os.environ.get("FLOWOXIDE_TASK_RUNNER_PROCESS_POOL_MAX_WORKERS")
        if env and env.strip().isdigit():
            return max(1, int(env.strip()))
...
def default_task_runner_from_env() -> MapTaskRunner | ProcessPoolTaskRunner:
    kind = os.environ.get("FLOWOXIDE_TASK_RUNNER", "thread").lower().strip()
