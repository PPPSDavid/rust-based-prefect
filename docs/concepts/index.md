...
FlowOxide splits work into a **Rust orchestration kernel** (`rust-engine`) and **Prefect-shaped Python authoring** (`prefect_compat`). The kernel owns durable flow/task state and valid transitions; Python runs your `@flow` / `@task` code and calls into the control plane.

Use these pages to learn the FlowOxide model end-to-end:
