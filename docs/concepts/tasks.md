...
- **`task.submit(*args, wait_for=...)`** — under **`ThreadPoolTaskRunner`** / **`ProcessPoolTaskRunner`**, return a **future** immediately after creating a **PENDING** task run; use **`future.result()`** / **`future.wait()`** or **`wait([...])`** to block inside the flow function. Independent submits can overlap. **`wait_for`** and tag-slot acquire gate **worker start** (not the submit call return). **`wait_for`** also accepts **`SubflowFuture`** from deployment-backed subflows — see **[How to compose flows with subflows](../how-to/subflows.md)**.
...
- **`SequentialTaskRunner`** keeps `submit` end-to-end synchronous (no overlap). **`ProcessPoolTaskRunner`** concurrent `submit` requires a **picklable** task body (same constraint as process `map`).
