...
2. **Deployment-backed (subflow as task)** — `deployment_ref("deployment-name").submit(...).result()` enqueues work on a deployment’s work pool. Returns a **`SubflowFuture`** that works with **`wait_for`** / **`wait`**.
