...
- Status: Hybrid MVP in active use — deployments (schedules, CLI/YAML Tier 1); **subflows M1+M2** on `main` (#34/#36) with user guide at `docs/how-to/subflows.md`; transition hooks; **global + tag concurrency limits** (`docs/how-to/concurrency-limits.md`); concurrent `task.submit` via ThreadPoolTaskRunner; agent tooling under `.cursor/` + `docs/agent/`. **Self-hosted:** Tier A (server Docker) + Tier C (basic auth) on `main`; Tier B0 store extract on `main` (#49). **Active plans:** Tier B1 Postgres — `IRONFLOW_DATABASE_URL` + Rust claim bind (`docs/plans/self-hosted-docker-tier-b.md`); **flow-run final state** proposed — default child aggregation (`wait_all`) + detach/explicit escape (`docs/plans/flow-run-final-state.md`). Concurrency plan implemented — `docs/plans/concurrency-limits.md`.
