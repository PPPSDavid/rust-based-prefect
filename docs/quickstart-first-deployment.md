...
The embedded local worker and scheduler are enabled by default (`IRONFLOW_ENABLE_LOCAL_WORKER=1`, `IRONFLOW_ENABLE_SCHEDULER=1`). See [Environment variables](reference/env-vars.md) to disable them for split-process workers.
...
- [How to deploy with the CLI and ironflow.yaml](how-to/deploy-with-cli.md) — manifest schema, `serve`, standalone workers.
