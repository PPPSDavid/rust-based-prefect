...
**Status:** Design accepted for backlog (from gap-canvas review) — not implemented  
...
> See also the top-of-doc **Implementer note** — same finding, placed for session kickoff.
...
- **Code today:** default runner is thread-pool (`IRONFLOW_TASK_RUNNER` / `ThreadPoolTaskRunner`); process pool exists for `map` but independent `submit()` is still sync on the coordinating thread — terminate design must extend isolation to the tasks we need to kill, not assume process-pool `map` alone is enough.
