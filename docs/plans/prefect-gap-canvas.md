...
**Status:** Proposal for review — **P0 landed** ([#61](https://github.com/PPPSDavid/rust-based-prefect/pull/61)); P1 next; **P3/P4**, **P3.2 lifecycle**, **PU UI**, **PH transition hooks** accepted  
**Date:** 2026-07-25 (rev: P0 landed)  
...
| Lifecycle hooks | `on_running` / `on_failure` / … | **`transition_hooks` / `on_transition(X→Y)` already on `@flow`/`@task`** | Deliberate API shape; **PH** = discoverability, sugar, worker coverage |
...
| Interactive pause / input | Temporal `gate` only today | gap → **P3.2 plan** | Operator pause `drain`/`terminate` + resume (not Prefect human-input forms — those stay park) |
| Logging (`get_run_logger`, `log_prints`) | Log list API exists; no Prefect logger helpers | gap | High DX value for porting |
...
| UI | Runs/Flows/Deployments/Work pools + DAG | partial | Surfaces exist; **aesthetics + ops-scale UX lag** — priority track **PU** ([`ui-ops-experience.md`](ui-ops-experience.md)); checklist stale; concurrency admin missing |
...
| ID | Finding | Severity |
| --- | --- | --- |
| D1 | `docs/gen_llms.py` sitemap lags `mkdocs.yml` / how-to index | **Fixed in P0 / #61** (re-check after Pages rebuild) |
| D2 | concurrency how-to missing from `mkdocs.yml` nav | **Fixed in P0 / #61** |
| D3 | No published concept pages for deployments, schedules, work pools, concurrency, server — Prefect has first-class concept pages; IronFlow buries them in how-tos / SELF_HOSTED | Medium (still open → P3.3) |
| D4 | `docs/ui_prefect_parity_checklist.md` checklist drift | **Fixed in P0.3 / #61** |
| D5 | `COMPATIBILITY.md` under-listed open gaps | **Fixed in P0.2 / #61** (keep canvas in sync) |
| D6 | Port guide missing common Prefect API table | **Fixed in P0.4 / #61** |
...
**Consensus order (updated after review):** Docs hygiene (P0) → finish resume (P1) → **runtime DX (P3)** + **UI ops (PU)** + **transition-hook polish (PH)** in parallel → **concurrency (P4)** → self-hosted follow-ups (P2) → later bets (P5/P7) → park Cloud/integrations.
...
### P0 — Docs truth & discoverability — **LANDED**

Shipped in [#61](https://github.com/PPPSDavid/rust-based-prefect/pull/61) (2026-07-25). Keep this section as history; do not re-open unless Pages/`llms.txt` drift returns.

| ID | Status | Notes |
| --- | --- | --- |
| **P0.1** | Done | `mkdocs.yml` + `docs/gen_llms.py` include concurrency/Docker/Compose/Postgres/workers/auth |
| **P0.2** | Done | `COMPATIBILITY.md` + mapping expanded; deliberate park separated |
| **P0.3** | Done | `docs/ui_prefect_parity_checklist.md` audited vs `frontend/` |
| **P0.4** | Done | Port guide “common Prefect APIs → IronFlow status” table |
...
| ID | Action | Acceptance | Ownership |
| --- | --- | --- | --- |
| **P3.0** | Stable **runtime context** helpers (`flow_run_id`, `task_run_id`, deployment id/name, parameters) — foundation for logging/cancel | `from prefect_compat import get_run_context` (name TBD) works in flow+task; tests; exported | shim |
| **P3.1** | Prefect-shaped **logging**: `get_run_logger()` (+ optional `log_prints=` on `@flow`/`@task`) → existing log rows / UI Logs tab | User `logger.info(...)` appears in `/api/flow-runs/{id}/logs` + UI; levels; task association | shim (+ tiny UI if needed) |
| **P3.2** | **Lifecycle control (expanded):** (1) **Cancel terminates** running tasks (killable workers, not poll-only); (2) **Pause `drain`** — pause scheduling, let in-flight finish; (3) **Pause `terminate`** — hard brake, kill in-flight, resume retries interrupted. Modes explicit in API/UI. Plan: [`flow-run-lifecycle-control.md`](flow-run-lifecycle-control.md) | Cancel stops blind `sleep` in process workers; pause requires `mode=drain\|terminate`; resume/retry semantics tested; docs/UI labels unmistakable | shim + engine + frontend |
...
### PH — Custom state transition hooks (priority; core already shipped)

Plan: [`transition-hooks-priority.md`](transition-hooks-priority.md).

**Already on `main`:** define `transition_hooks=[on_transition(fn, from_state=…, to_state=…)]` on `@flow` / `@task` — when the run goes **X → Y**, run **Z** (`fn(TransitionContext)`). Wildcards via `None`. In-process, post-commit, non-fatal on exception.

| ID | Action | Acceptance | Ownership |
| --- | --- | --- | --- |
| **PH.0** | How-to + port-guide + copy-paste example (X→Y→Z on definition) | Discoverable without reading COMPATIBILITY | docs + shim |
| **PH.1** | Ergonomic sugar (`on_failure=` / `on_completion=` and/or decorator) wrapping `on_transition` | Same semantics; both styles documented | shim |
| **PH.2** | Audit hooks on process-pool + deployment/HTTP workers; fix or document limits | How-to matrix accurate | shim |
| **PH.3** | Hook examples/tests for P3.2 pause/cancel/resume edges | `→PAUSED` / `→CANCELLED` covered | shim |
| **PH.4** | Optional non-blocking/async dispatch design | Design note; default stays sync | shim |

### PU — UI aesthetics & ops-scale experience (priority; parallel track)

Plan: [`ui-ops-experience.md`](ui-ops-experience.md). **Method:** each pass implements a slice → agent renders/screenshots with seeded data → **you confirm** → next pass. Not a Prefect pixel clone.

| ID | Action | Acceptance | Ownership |
| --- | --- | --- | --- |
| **PU.0** | Baseline screenshot audit + refresh functional checklist truth | Artifact pack; `ui_prefect_parity_checklist.md` matches reality | frontend |
| **PU.1** | Visual foundation (tokens, type, density, shell) | Maintainer confirms direction | frontend |
| **PU.2** | Runs triage at hundreds-of-rows scale (filter/search/density/attention) | Seeded ≥200 runs usable | frontend (+ API if filters missing) |
| **PU.3** | Flows/deployments catalog at scale | Many entities searchable/scannable | frontend |
| **PU.4** | Run detail operator chrome (lifecycle actions once P3.2 exists) | Clear drain/terminate/cancel/resume | frontend |
| **PU.5** | Bulk fleet actions (subset) | Confirm dialogs; after single-run lifecycle | frontend + API |
| **PU.6** | Concurrency admin (align with **P4.3**) | Same design system as PU.1 | frontend |
| **PU.7** | Empty/loading/error + light keyboard polish | No blank confusing states | frontend |
...
| **P3.2f** | How-to + MEMORY_BANK; export cooperative helpers as optional aid |
...
0. **P0.1–P0.4** — Docs truth & discoverability ([#61](https://github.com/PPPSDavid/rust-based-prefect/pull/61))  

**Now / next:**

1. **P1.1** — Merge/finish task resume Goal A (#50)  
2. **P1.2** — Resume hardening + UI skipped-state  

**Core DX train (after P1):**

3. **P3.0 + P3.1** — Runtime context + `get_run_logger` (+ optional `log_prints`)  
4. **P3.2a → P3.2f** — Lifecycle control: force cancel + pause `drain`/`terminate` + resume ([plan](flow-run-lifecycle-control.md))  
5. **P4.0** — GCL leases correct under cancel/pause/resume  
6. **P4.2** — CLI `ironflow gcl`  
7. **P4.3** / **PU.6** — UI concurrency admin (same design tokens as PU.1)  
8. **P4.1** — Async concurrency helpers  
9. **P4.4** — perf_matrix gcl/tag recipe  

**UI priority track (parallel; confirm between passes):**

10. **PU.0** — Baseline audit + screenshots ([plan](ui-ops-experience.md))  
11. **PU.1** — Visual foundation → **stop for your confirm**  
12. **PU.2 → PU.3** — Runs/flows ops at scale → confirm each pass  
13. **PU.4** — Run detail lifecycle chrome (after P3.2 APIs)  
14. **PU.5 / PU.7** — Bulk actions + empty/error polish as needed  

**Transition hooks priority (parallel; core exists):**

15. **PH.0** — How-to + examples for X→Y→Z on `@flow`/`@task` ([plan](transition-hooks-priority.md))  
16. **PH.1** — Ergonomic sugar (`on_failure=` / etc.)  
17. **PH.2** — Worker/deployment path audit  
18. **PH.3** — Lifecycle edges after P3.2  

**Parallel / light docs:**

19. **P3.3 + P3.4** — Concept pages, retries/timeouts truth  
20. **P3.5** — Logging + lifecycle smoke (drain / terminate / cancel)  

**After core DX:**

21. **P2.1** — Postgres Rust schedule/gate (and other Tier B follow-ups)  
22. **P5.1** or **P7.1** — Cache Goal B vs automations design  
