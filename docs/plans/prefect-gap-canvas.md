...
**Status:** Proposal for review — P0/P1 agents in flight; **P3/P4 expanded**; **P3.2 lifecycle redesign accepted**  
**Date:** 2026-07-25 (rev: P3.2 cancel/pause/resume)  
...
| **P3.2** | **Lifecycle control (expanded):** (1) **Cancel terminates** running tasks (killable workers, not poll-only); (2) **Pause `drain`** — pause scheduling, let in-flight finish; (3) **Pause `terminate`** — hard brake, kill in-flight, resume retries interrupted. Modes explicit in API/UI. Plan: [`flow-run-lifecycle-control.md`](flow-run-lifecycle-control.md) | 🟨 P3.2a/b: mode-required pause + drain/resume + cancel metadata; **P3.2c** process kill + UI chooser still open | shim + engine + frontend |
