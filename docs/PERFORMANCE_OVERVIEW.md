...
Script: `benchmarks/compare_prefect_vs_flowoxide.py` (run from the repository root)
...
| --- | --- |
| **`flowoxide`** | In-process `prefect_compat` + control plane (no HTTP). |
| **`flowoxide_http`** | Same stack behind the local HTTP API boundary (adds request/serialization cost). |
...
| Pattern | Complexity | FlowOxide (in-proc) | FlowOxide + HTTP | Prefect OSS (local) |
| --- | ---: | ---: | ---: | ---: |
...
**Approximate multipliers** (FlowOxide in-proc **÷** Prefect, same cell):
...
**HTTP boundary:** FlowOxide through the local API is still **roughly one to two orders of magnitude** above Prefect on this harness (e.g. mapped @100 ~**110×**), because the comparison is dominated by orchestration cost, not network latency to a remote cluster.
...
3. **Your code dominates:** If 99% of wall time is inside task bodies (database, APIs, CPU work), **end-to-end** speedup may be **modest** even when FlowOxide is much faster at scheduling.
4. **Subset product:** FlowOxide does **not** implement full Prefect; faster orchestration only matters where you stay inside the supported subset ([Compatibility](compatibility.md)).

## Regression tracking (FlowOxide vs FlowOxide)

Contributors who compare FlowOxide builds over time use the deterministic `perf_matrix.py` harness; methodology is documented in the repository at `docs/perf_methodology.md` (not required reading to **use** FlowOxide).
