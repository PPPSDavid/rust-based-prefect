...
| `task.submit()` / futures | Supported: dependency chains, `wait_for`, and concurrent independent submits under `ThreadPoolTaskRunner`. Process-pool submit stays synchronous (map for process fan-out). |
