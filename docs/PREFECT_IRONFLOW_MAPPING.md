...
| `task.submit()` / futures | Supported: dependency chains, `wait_for`, and concurrent independent submits under `ThreadPoolTaskRunner` / `ProcessPoolTaskRunner` (PENDING + future return immediately; wait/acquire/body deferred to workers). Sequential runner stays sync. Default flow finalization is **`wait_all`** (aggregate child task states in Rust); use `detach=True` or `@flow(final_state="explicit")` for fire-and-forget / body-driven completion. |
