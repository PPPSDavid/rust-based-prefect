...
| State hooks (`on_running`, …) | IronFlow uses **`transition_hooks`** on `@flow` / `@task` with `TransitionHookSpec` / `on_transition`—see `COMPATIBILITY.md`. Returning `None` matches Prefect notify-style hooks. Returning a `RunState` / `TransitionDecision` can rewrite a proposed `RUNNING`→terminal destination before commit (IronFlow extension; Prefect 3 hooks cannot do this). |
