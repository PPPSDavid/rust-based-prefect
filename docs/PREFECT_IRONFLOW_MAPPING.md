...
| State hooks (`on_running`, …) | IronFlow uses **`transition_hooks`** on `@flow` / `@task` with `TransitionHookSpec` / `on_transition`—see `COMPATIBILITY.md`. Observe mode matches Prefect notify-style hooks. **`mode="rewrite"`** can change a proposed `RUNNING`→terminal destination before commit (IronFlow extension; Prefect 3 hooks cannot do this). |
