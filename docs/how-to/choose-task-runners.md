...
| **`ProcessPoolTaskRunner`** | Non-blocking (picklable bodies in child processes) | Concurrent via process pool (picklable tasks) |

Independent branches can use either multiple **`submit()`** calls or **`map()`**. Use **`wait_for`** so dependents do not start early (`wait_for` gates the worker, not the submit return).
...
| Run CPU-heavy pure Python (numeric work, compression, …) | **`ProcessPoolTaskRunner`** | Picklable top-level task + `heavy.submit(...)` / `heavy.map(items)` |
...
Use when task bodies are **CPU-heavy** and can run in child processes. The callable must be **picklable** — in practice, use a **top-level function** in an importable module (see `prefect_compat.mp_picklable` and `python-shim/tests/test_task_runners.py`). Both **`submit()`** and **`map()`** use the process pool for bodies.
...
- **Windows** multiprocessing from pytest is unreliable; process-pool `submit` / `map` is primarily validated on Linux/macOS.
...
1. **Non-picklable process-pool tasks** — closures, lambdas, and tasks defined only inside test modules often fail to pickle. Use a top-level callable in an importable module.
...
5. **Omitting `wait_for` on dependents** — without `wait_for` (or resolving upstream futures as args), concurrent submits may race; gate with `wait_for=[upstream]` (gates worker start, not the submit return).
