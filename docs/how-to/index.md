...
- **[How to set up FlowOxide](setup.md)** — clone, Python environment, build the Rust engine, environment variables, `PYTHONPATH`.
- **[How to run the server and UI](server-and-ui.md)** — API + optional Vite UI via `scripts/flowoxide_server.py`.
- **[How to run the server in Docker](docker-quickstart.md)** — single-container image, volumes, optional basic auth.
- **[How to run FlowOxide with Docker Compose](docker-compose.md)** — Postgres + API + services + HTTP workers.
- **[How to run background services](run-background-services.md)** — `flowoxide server services start` without an HTTP listener.
- **[How to use Postgres for the control plane](database-postgres.md)** — `FLOWOXIDE_DATABASE_URL`, Rust claim bind.
- **[How to run workers in HTTP mode](worker-http-mode.md)** — `FLOWOXIDE_WORKER_MODE=http`, claim API, no shared DB volume.
- **[How to secure a self-hosted server](secure-self-hosted.md)** — `FLOWOXIDE_*_AUTH_STRING` Basic auth and reverse-proxy notes.
- **[How to deploy with the CLI and `flowoxide.yaml`](deploy-with-cli.md)** — `flowoxide init` / `deploy` / `serve` / `worker start`, manifest schema, and `deploy()` / `serve()` Python API.
