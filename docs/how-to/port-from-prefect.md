...
| `on_running` / `on_failure` / … hooks | deliberate | Map notify logic to `transition_hooks=` + `on_transition(...)` observe edges. To change the recorded terminal, use `mode="rewrite"` + `TransitionDecision` (IronFlow extension; not Prefect hook behavior). |
