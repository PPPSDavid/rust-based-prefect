...
4. **Rename hooks if you use them** — Prefect’s named lifecycle hooks are not mirrored literally. Use IronFlow **`transition_hooks`** with **`TransitionHookSpec`** / **`on_transition(fn, from_state=…, to_state=…)`** on `@flow` / `@task` (X→Y→do Z). See **[Flows](../concepts/flows.md)**, the compatibility matrix, and polish plan **`docs/plans/transition-hooks-priority.md`**.
