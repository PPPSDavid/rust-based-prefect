...
| --- | --- | --- |
| `from prefect import flow` / rename a flow | deliberate | **Source first:** `@flow(name="B", formerly=["A"])` + `ironflow deploy --all --prune`. UI rename is catalog-only when no served deployment remains. Prefect cannot rename in place. Guide: [rename-archive-flows](rename-archive-flows.md) |
